...
from YummiMusic.misc import SUDOERS, db
from YummiMusic.utils.database import (is_active_chat,
...
from YummiMusic.utils.formatters import seconds_to_min
from YummiMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from YummiMusic.utils.stream.autoclear import auto_clean
from YummiMusic.utils.thumbnails import gen_thumb
    ...
    if command == "Pause":
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Yummi.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Yummi.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Yummi.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
